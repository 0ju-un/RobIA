...
class Aggregation(SubModule):
    def __init__(self,
        ...
        self.conv_stem = BasicConv(matching_head, 8,
                                   is_3d=True, kernel_size=3, stride=1, padding=1, depthwise=depthwise)
        ...
        for i in range(3):
            ...
            for n in range(block_n[i]):
                ...
                conv.append(
                    BasicConv(
                        ...
                        relu=relu, kernel_size=kernel_size, padding=padding, stride=stride, dilation=dilation, depthwise=depthwise))
                inp = channels[i+1]
            ...
            self.conv_up.append(
                BasicConv(
                    ...
                    relu=relu, kernel_size=(4, 4, 4), padding=(1, 1, 1), stride=(s_disp, 2, 2), depthwise=depthwise))

            self.conv_agg.append(nn.Sequential(
                BasicConv(
                    channels[i], channels[i], is_3d=True, kernel_size=3, padding=1, stride=1, depthwise=depthwise),
                BasicConv(
                    channels[i], channels[i], is_3d=True, kernel_size=3, padding=1, stride=1, depthwise=depthwise)))

            self.conv_skip.append(BasicConv(2*channels[i], channels[i], is_3d=True, kernel_size=1, padding=0, stride=1, depthwise=depthwise))
